...
class BlackHole2D(Equation):
    ...
    def loop(self, d_x, d_y, d_idx, d_au, d_av, t):
        d_au[d_idx] += -self.M * d_x[d_idx] / pow((d_x[d_idx]**2 + self.soft**2 + (d_y[d_idx] + self.t_hit)**2),3.0/2.0)
        d_av[d_idx] += -self.M * (d_y[d_idx] + self.t_hit) / pow((d_x[d_idx]**2 + self.soft**2 + (d_y[d_idx] + self.t_hit)**2),3.0/2.0)
