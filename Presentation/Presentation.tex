...
In order to simulate the collision smoothed particle hydrodynamics (SPH) was used.
\begin{itemize}
\item Developed in 1977 for astrophysics simulations
\item Mesh-free, $N$-body methods
\item Uses nearest neighbours
\end{itemize}
The neutron star is treated as a $120 \times 15$ tank with a particle spacing of $0.075$. The simulation was run in parallel over three days with PySPH, a SPH framework.
}
...
The energy can be calculated in the same fashion as before:
\begin{align*}
E &= \sum_i \frac{1}{2} m_i \left( u_i^2 + v_i^2 \right) + m_i g y_i.
\end{align*}
However, this must be transformed to three dimensions:
\begin{align*}
...
If this project were continued some modifications would be
\begin{itemize}
\item Better initial conditions for the simulation
\item Use a spherical model instead of planar
\item Use general relativity instead of Newtonian gravity
\end{itemize}
}
